...
class ShapedCodecTests(unittest.TestCase):
    def test_tthresh_shaped_adapter_roundtrips_selected_orientation(self):
        class FakeTTHRESH:
            decoded_by_token = {}
            call_count = 0
            ...
            def compress(cls, data, *, relative_error):
                self.assertEqual(relative_error, 1e-3)
            ...
            def decompress(cls, payload):
                token = int(np.frombuffer(bytes(payload), dtype=np.uint8)[0])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "src.shaped_codecs.load_tthresh",
    ...
    def test_qoz_shaped_adapter_roundtrips_selected_orientation(self):
        class FakeQoZ:
            decoded_by_token = {}
            call_count = 0
            ...
            def compress(cls, data, error_bound, mode):
                self.assertEqual(error_bound, 0.01)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            values.tofile(raw_path)
            with patch(
                "src.shaped_codecs.load_qoz",
                return_value=FakeQoZ,
            ):
                field = compress_shaped_lossy_raw(
                    "qoz",
                    str(raw_path),
